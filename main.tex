  ...
  \begin{itemize}
    ...
    \begin{figure}[h]
      \begin{tikzpicture}[scale=1.3, block/.style = {rectangle, draw=black, thick, text width=4em, align=center, rounded corners, minimum height=2em},
        ...
        \draw (1.5, -0.5) node[tx] (T1) {\textrm{Tx.1}};
        \draw (4.5, -0.5) node[tx] (T2) {\textrm{Tx.2}};
        ...
        \draw (1.62, -1.7) node {$\text{eval}($};
        \draw (2.5, -1.7) node (E1) {\ScriptSig, };
        \draw (4.05, -1.7) node (E2) {\ScriptPubKey$)$};
    ...
    \item<+-> マイナーは$\text{eval}$の結果が$\SOne{0}$以外なら受理し、
    $\SOne{0}$ならば拒否する
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \pause
  \begin{enumerate}
    \item<+-> 今、チーム$T_i$が問題$j$のフラッグワード$F_j$を得た。
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    \begin{column}{0.4\textwidth}
\begin{lstlisting}[style=bitcoin-script]
OP_DUP
$1$
OP_EQUAL
OP_IF
...
\end{lstlisting}
    ...
    \begin{column}{0.6\textwidth}
      \begin{enumerate}
        ...
        \item<+-> スタックの先頭を捨てる$\STwo{h_{1j},S_1}$ \pause
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<+-> Bitcoinのリファレンス実装では、
    提案するCTFで利用するようなトランザクションを受け付けない。
    従って現実的にはEthereumなど別の暗号通貨を利用しなければならないかもしれない
